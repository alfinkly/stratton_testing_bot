...
from data.variables import TEST_APPOINT_TO_USER, TEST_APPOINT_TO_ADMIN
...
async def set_time(message: Message,
    ...
    user = await orm.user_repo.upsert_user(
        user_id=user.user_id,
        test_status=TestStatus.waiting,
    ...
    await message.answer(text=TEST_APPOINT_TO_USER.format(text_date, text_time))
    tga = TgActions(message.bot, *env.admins)
    await tga.send_message(
        text=TEST_APPOINT_TO_ADMIN.format(
            user.username,
            user.fullname,
            text_date,
            text_time,
...
async def time_select(callback: CallbackQuery, callback_data: TimeCallback,
                      state: FSMContext, user: User, orm: ORM,
                      scheduler: AsyncIOScheduler, env: Environ):
    data = await state.get_data()
    date: datetime.datetime = data["testing_at"]
    testing_at = date.replace(hour=callback_data.hour,
                              minute=callback_data.minute)

    if is_time_over(testing_at):
        return await callback.message.answer(
            text="Указанное время уже прошло ⌛️")

    await orm.user_repo.upsert_user(user.user_id,
                                    test_status=TestStatus.waiting,
                                    testing_at=testing_at)
    ...
    await callback.message.edit_text(
        text=TEST_APPOINT_TO_USER.format(text_date, text_time),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[]))
    tga = TgActions(callback.bot, *env.admins)
    await tga.send_message(
        text=TEST_APPOINT_TO_ADMIN.format(user.username, user.fullname,
                                          text_date, text_time,
                                          user.phone_number))
